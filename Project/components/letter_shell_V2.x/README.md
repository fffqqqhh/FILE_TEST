...
1. 定义shell对象（已完成，shell.c定义，shell.h声明）

    ```C
    SHELL_TypeDef shell;
    ```

2. 定义shell读，写函数，函数原型如下（已完成，shell.h）
...
3. 调用shellInit进行初始化
    ...
    ```C
    shell.read = shellRead;		// 当使用中断方式接收数据时，不需要绑定shell->read，但需要在中断中调用shellInput传递接收数据
...
4. 调用(建立)shell任务
    ...
    OsTaskCreate(shellTask, &shell, ...);
	  xTaskCreate(shellTask, "shellTask", 128, &shell, tskIDLE_PRIORITY + 2, NULL);	// FreeRTOS Task Create demo
    ```
...
5. 说明
   ...
   - 对于在无操作系统环境下，可以使用查询的方式，使能```SHELL_USING_TASK```，然后在循环中不断调用shellTask
   - 对于使用操作系统的情况，使能```SHELL_USING_TASK```和```SHELL_TASK_WHILE```宏，然后创建shellTask任务
...
7. 配置宏
    ...
    | SHELL_USING_VAR            | 是否使用变量功能               |
    | SHELL_USING_SHOW_ADDR      | 是否使用地址显示功能           |
    ...
    | SHELL_PRINT_BUFFER         | shell格式化输出的缓冲大小       |
    | SHELL_GET_TICK()           | 获取系统时间(ms)               |
    | SHELL_DOUBLE_CLICK_TIME    | 双击间隔(ms)                   |
    | SHELL_UNITE_PRINTF         | 转发字符串到其它输出通道打印     |
...
- 2024/04/12 1.0.0(2.0.8a) by:CAIJIAMING

  - 2024/04/03 修复README错误，更新移植描述
  - 2024/04/03 简化移植过程，不使用默认命令变量表
  - 2024/04/03 新增CTAL_2~CTAL~8组合键（键值来自MobaXterm）
  - 2024/04/03 默认命令定义使用兼容性更好的命令表方式
  - 2024/04/03 默认关闭shell格式输出功能
  
  - 2024/04/07 默认开启长帮助、双击Tab功能
  - 2024/04/07 默认关闭变量功能
  - 2024/04/07 新增自定义按键反馈提示词hint
  - 2024/04/07 不显示cls命令的函数返回值
  - 2024/04/07 新增双击CTAL键（键值来自MobaXterm）

  - 2024/04/12 新增地址显示功能，可现实命令和变量地址，通过SHELL_USING_SHOW_ADDR宏开启

  - 2024/05/10 最大shell数量调整为1

- 2024/12/10 1.1.0 by:CAIJIAMING

  - 2024/09/11 优化cfg文件，增加注释说明，增加3个移植方案介绍
  - 2024/09/11 新增SHELL_UNITE_PRINTF宏，可转发字符串到其它输出通道打印(如BLE)
  - 2024/12/03 更新README，增加使用说明
  - 2024/12/03 优化警告提示
  - 2024/12/03 直接使用'$'符号来写变量，例如：写val_name：$temp 123
  - 2024/12/10 新增shellDisplayAddr函数，解决地址显示功能只能通过pringf的方式打印的问题
