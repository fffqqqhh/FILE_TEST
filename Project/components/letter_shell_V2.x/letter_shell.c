...
#define	TEXT_INFO			"\r\n"															    \
                            ...
                            "+=========================================================+\r\n"
#define	TEXT_PWD_HINT      	"\r\nPlease input password:"
#define	TEXT_PWD_RIGHT     	"\r\npassword confirm success.\r\n"
#define	TEXT_PWD_ERROR     	"\r\npassword confirm failed.\r\n"
#define	TEXT_FUN_LIST      	"\r\nCOMMAND LIST:\r\n\r\n"
#define	TEXT_VAR_LIST      	"\r\nVARIABLE LIST:\r\n\r\n"
#define	TEXT_CMD_NONE      	"Command not found\r\n"
#define	TEXT_CMD_TOO_LONG  	"\r\nWarnig: Command is too long\r\n"
#define	TEXT_READ_NOT_DEF  	"error: shell.read must be defined\r\n"
...
void shellSetVariable(SHELL_TypeDef *shell, char *var, int value);
...
#if SHELL_USING_VAR == 1
unsigned char defaultVariable;		// 注册到shell默认变量表，防止变量表为空编译不通过
/**
* @brief shell默认变量表
* 
*/
...
{
	SHELL_VAR_ITEM_CHAR(defaultVariable, defaultVariable, Test Variable)
};
#endif /** SHELL_USING_VAR == 1 */
...
{
    {NULL, SHELL_KEY_LF,          shellEnter},
    {NULL, SHELL_KEY_CR,          shellEnter},
    {NULL, SHELL_KEY_TAB,         shellTab},
    {NULL, SHELL_KEY_BACKSPACE,   shellBackspace},
    {NULL, SHELL_KEY_DELETE,      shellBackspace},
    {NULL, SHELL_KEY_ESC,         shellAnsiStart},
};
...
void shellInit(SHELL_TypeDef *shell)
{
    ...
    shellDisplay(shell, TEXT_PWD_HINT);
#else
    shellDisplay(shell, TEXT_INFO);
...
unsigned short shellDisplay(SHELL_TypeDef *shell, const char *string)
{
    ...
    SHELL_UNITE_PRINTF(string - count, count);
...
static void shellDisplayValue(SHELL_TypeDef *shell, int value)
{
    ...
    // char i = 10;
    int i = 10;     // ESP32 IDF平台不支持用char的变量作为数组下标
    char tmp;
 ...
 * @param addr 地址
 */
static void shellDisplayAddr(SHELL_TypeDef *shell, uint32_t addr)
{
    char str[11] = {0};
   
    // sprintf((char *)&str, "0x%08x", addr);
    sprintf((char *)&str, "0x%04x%04x", (uint16_t)(addr>>16), (uint16_t)(addr&0xFFFF));     // ESP32 IDF平台不支持用%x打印uint32_t
...
static void shellEnter(SHELL_TypeDef *shell)
{
    ...
    if(shell->status.authFlag == 0)
    {
        ...
        {
            shellDisplay(shell, TEXT_PWD_ERROR);
            shellDisplay(shell, TEXT_PWD_HINT);
        }
        else
        {
            shell->status.authFlag = 1;
            shellDisplay(shell, TEXT_PWD_RIGHT);
            shellDisplay(shell, TEXT_INFO);
            shellDisplay(shell, shell->command);
        }
    ...
    *(shell->buffer + shell->length++) = 0;
    ...
    {        
        if(paramCount == 2)
        {
            shellSetVariable(shell, shell->param[0], shellExtParsePara(shell->param[1]));
        }
        else
        {
    ...
    for (unsigned short i = 0; i < shell->commandNumber; i++)
    {
        if (strcmp((const char *)shell->param[0], (base + i)->name) == 0)
        {
            ...
            shell->status.isActive = 0;
        #if SHELL_DISPLAY_RETURN == 1
        if (strcmp((const char *)shell->param[0], "cls") != 0)      // 只显示非cls命令的函数返回值
        {
    ...
    if (runFlag == 0)
    {
        shellDisplay(shell, TEXT_CMD_NONE);
    }
    shellDisplay(shell, shell->command);
}
...
static void shellNormal(SHELL_TypeDef *shell, char data)
{
    ...
    {
        shellDisplay(shell, TEXT_CMD_TOO_LONG);
        shellDisplay(shell, shell->command);
        shellDisplay(shell, shell->buffer);
        shell->cursor = shell->length;
    }
        ...
        if (shell->status.authFlag == 1)
        {
            ...
            for (short i = 0; i < shell->keyFuncNumber; i++)
            {
                if (base[i].keyCode == data) {
                    if (base[i].function) {
                        base[i].function(shell);
                        
                        shellDisplay(shell, base[i].hint);
                        shellDisplay(shell, shell->command);
                    }
                    keyDefFind = 1;
                }
        ...
        if (keyDefFind == 0)
        {
                ...
                i++)
            {
                if (shellDefaultKeyFunctionList[i].keyCode == data) {
                    if (shellDefaultKeyFunctionList[i].function) {
                        shellDefaultKeyFunctionList[i].function(shell);
                    }
                    keyDefFind = 1;
                }
            }
...
void shellTask(void *param)
{
    ...
    if (shell->read == NULL)
    {
        shellDisplay(shell, TEXT_READ_NOT_DEF);
        while (1) ;
    }
...
void shellListVariables(void)
{
    ...
    shellDisplay(shell, TEXT_VAR_LIST);   

    for (short i = 0; i <  shell->variableNumber; i++)
    {
#if SHELL_USING_SHOW_ADDR == 1
        // 显示变量地址
        shellDisplayAddr(shell, (unsigned int)(uintptr_t)((base + i)->value));
        shellDisplay(shell, ": ");
#endif
...
void shellHelp(int argc, char *argv[])
{
    ...
    {
#endif /** SHELL_LONG_HELP == 1 */
        shellDisplay(shell, TEXT_FUN_LIST);       
        for(unsigned short i = 0; i < shell->commandNumber; i++)
        {
...
#if SHELL_USING_SHOW_ADDR == 1
                // 显示命令地址
                shellDisplayAddr(shell, (unsigned int)(uintptr_t)((base + i)->function));
				shellDisplay(shell, "\r\n");
#endif				
                shellDisplay(shell, (base + i)->desc);
                shellDisplay(shell, "\r\n");
