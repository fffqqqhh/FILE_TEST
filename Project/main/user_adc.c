...
void adc1_init(void)
{
    ...
    //-------------ADC1 Config---------------//
    ESP_ERROR_CHECK(adc_oneshot_config_channel(adc1_handle, NTC_ADC_CHAN0, &config));
...
void adc1_deinit(void)
{
    ...
    // deregister Curve Fitting calibration scheme
    if (do_calibration1_chan0) {
        do_calibration1_chan0 = false;
...
void adc_ntc_init(void)
{
    ...
    do_calibration1_chan0 = adc_calibration_init(ADC_UNIT_1, NTC_ADC_CHAN0, NTC_ADC_ATTEN, &adc1_cali_chan0_handle);
}
...
uint16_t adc_ntc_read(void)
{
    ...
    ESP_ERROR_CHECK(adc_oneshot_read(adc1_handle, NTC_ADC_CHAN0, &adc_raw));
